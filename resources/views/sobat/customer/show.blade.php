                ...
                <ul class="list-unstyled mb-4 mt-3">
                    ...
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-user text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Nama Customer:</span> <span>{{ textCamelCase($customer->nama_karyawan) }}</span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-map-pin text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Tempat Lahir:</span> <span>{{ textCamelCase($customer->tempat_lahir) }}</span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-calendar text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Tanggal Lahir:</span>
                        <span>{{ !empty($customer->tanggal_lahir) ? DateToIndo($customer->tanggal_lahir) : '' }}</span>
                    </li>
                    <li class="mb-3">
                        ...
                        <span mb-3>{{ textCamelCase($customer->alamat) }}</span>
                    </li>
                    ...
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-gender-genderfluid text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Jenis Kelamin:</span>
                        <span>{{ $customer->jenis_kelamin == 'L' ? 'Laki - Laki' : 'Perempuan' }}</span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                    ...
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-friends text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Status Kawin:</span>
                        <span>{{ $customer->keterangan_status_kawin }} </span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-school text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Pendidikan Terakhir:</span>
                        <span>{{ $customer->pendidikan_terakhir }} </span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-building text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Kantor:</span>
                        <span>{{ $customer->nama_cabang }} </span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-building-arch text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Depertemen:</span>
                        <span>{{ $customer->nama_dept }} </span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-user text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Jabatan:</span>
                        <span>{{ $customer->nama_jabatan }} </span>
                    </li>
                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-barcode text-heading"></i><span class="fw-medium mx-2 text-heading">Atasan:</span>
                        <span>{{ $customer->nama_atasan ?? '-' }}</span>
                    </li>

                    <li class="d-flex align-items-center mb-3">
                        <i class="ti ti-calendar text-heading"></i><span class="fw-medium mx-2 text-heading">
                            Tanggal Masuk:</span>
                        <span>{{ !empty($customer->tanggal_masuk) ? DateToIndo($customer->tanggal_masuk) : '' }}
                        </span>
                    </li>
                    @if ($customer->status_aktif_karyawan === '0')
                        <li class="d-flex align-items-center mb-3">
                            <i class="ti ti-calendar text-heading"></i><span class="fw-medium mx-2 text-heading">
                                Tanggal Nonaktif:</span>
                            <span>{{ !empty($customer->tanggal_nonaktif) ? DateToIndo($customer->tanggal_nonaktif) : '' }}
                            </span>
                        </li>
                        <li class="d-flex align-items-center mb-3">
                            <i class="ti ti-calendar text-heading"></i><span class="fw-medium mx-2 text-heading">
                                Tanggal Off Gaji:</span>
                            <span>{{ !empty($customer->tanggal_off_gaji) ? DateToIndo($customer->tanggal_off_gaji) : '' }}
                            </span>
                        </li>
                    @endif
                </ul>
            ...
            <div class="col-md-12">
                <div class="card card-action mb-4">
                    <div class="card-header align-items-center d-flex justify-content-between">
                        ...
                        <div>
                            <form id="formHapusSemuaWajah" method="POST" action="{{ route('facerecognition.destroyAll', Crypt::encrypt($customer->nik)) }}" style="display:inline">
                                @csrf
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (btnHapusSemua) {
            btnHapusSemua.addEventListener('click', function(e) {
                e.preventDefault();
                if (confirm('Yakin ingin menghapus SEMUA data wajah customer ini?')) {
...
<script>
    $("#btnAddface").click(function(e) {
        ...
        $("#loadmodal").load('/facerecognition/' + '{{ Crypt::encrypt($customer->nik) }}' + '/create');
    });
